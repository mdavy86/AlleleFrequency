...
setDTthreads(1)
module("load openlava asub/2.2 bwa/0.7.12 samtools/0.1.18 bcftools/1.10.2 vcftools")
```
...
#file.symlink(malepoolfiles, outDir)
```

```{r transform filenames to sample, lane and suffix for data.table from link}
malepooldt_bam <- as.data.table(grep(paste(".bam$", collapse = "|"), list.files(path="."), invert=FALSE, value=TRUE))
malepooldt_bam <- malepooldt_bam[, V2 := str_replace_all(V1, "_L" , "&_L")
               ][, V2 := str_replace_all(V2, "XX_" , "XX_&")
               ][, V2 := str_replace_all(V2, ".bam" , "&.bam")                 
               ][, c("HL55", "sample", "lane", "ftype") := tstrsplit(V2, "&", fixed=TRUE)
               ][, V2:=NULL]
malepooldt_bam
#subset malepool from parent pool files
fwrite(malepooldt_bam, 'malepooldt_bam.tsv', append=FALSE)
```

```{r index}
vcfin       <- paste(malepooldt_bam$HL55, malepooldt_bam$sample, malepooldt_bam$lane, malepooldt_bam$ftype, sep = "")
vcfin
for (i in vcfin){
vcfcmd      <- glue::glue("bcftools index -t {i}")
...
bamfiles    <- paste(malepooldt_bam$V1)
file.exists(bamfiles)
out         <- paste(malepooldt_bam$sample, "samtoolsmpile_chr25.vcf.gz", sep = "_")
...
bamfiles    <- paste(malepooldt_bam$HL55, malepooldt_bam$sample, malepooldt_bam$lane, malepooldt_bam$ftype, sep = "")
#bamfiles
out         <- paste(malepooldt_bam$sample, "_chr25_pile_.vcf.gz", sep = "_")
#out
...
malepooldt1 <- malepooldt_bam[, `:=` (vcf=".vcf.gz", chr="_chr25_", work="pile_", filterquery="f", "filterq"="withhom", "chr_bit"="1to10Mb_")]
fwrite(malepooldt1, 'malepooldt1.tsv', append=FALSE)
```

```{r allele info plugin}
malepooldt1 <- malepooldt_bam[, `:=` (infoplugin="infoplugin")]
vcfin       <- paste(malepooldt1$sample, malepooldt1$chr, malepooldt1$work, malepooldt1$vcf, sep = "")

for (i in vcfin){
vcfcmd      <- glue::glue("bcftools +fill-tags {i}  -o 'infoplugin{i}' ")
vcfcmd
logDir         <- "/powerplant/workspace/hrtcdf/github/FSTs/malepool/logDir"
bsub_out_file  <- file.path(logDir, "job.out"); bsub_err_file <- file.path(logDir, "job.err")
bsub_cmd       <- glue("bsub -n 8 -o {bsub_out_file} -e {bsub_err_file} {vcfcmd}")
...
```
```{r filter removing homs}
#vcfin       <- list.files(path = ".", pattern = "*chr25_pile.vcf.gz$")
vcfin       <- paste(malepooldt1$sample, malepooldt1$chr, malepooldt1$work, malepooldt1$vcf, sep = "")
for (i in vcfin){
vcfcmd      <- glue::glue("bcftools query {i} -e 'INFO/DP<10 || INFO/DP>120 || INFO/AD[0]=INFO/AD[1] || INFO/AD[1]<10' -f '[[%SAMPLE]]\t%CHROM\t%POS\t%REF\t%ALT\t%DP\t%AD\t%AD{{0}}\t%AD{{1}}\t%AD{{2}}\t%AD{{3}}\t%AC\t%AN [[\t%GT]]\n' -o 'f{i}' ")
vcfcmd
logDir         <- "/powerplant/workspace/hrtcdf/github/FSTs/malepool/logDir"
bsub_out_file  <- file.path(logDir, "job.out"); bsub_err_file <- file.path(logDir, "job.err")
bsub_cmd       <- glue("bsub -n 8 -o {bsub_out_file} -e {bsub_err_file} {vcfcmd}")
...
files      <-  paste("withhom_", malepooldt1$sample, "_chr25_pile_.vcf.gz", sep = "")
files
...
#clean up unneeded files in RAM
...
listpercentmale <- c("90% male", "75% male", "60% male", "80% male", "50% male", "45% male", "32% male", "21% male", "10% male")
datonlyrualt <- lapply(datmerged, function(x) x[!is.na(mpSAMPLE) & mpALT
