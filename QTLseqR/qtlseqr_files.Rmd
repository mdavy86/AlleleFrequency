...
Russell_tp_bams <- Rualn_bams[V1 =='ER10522_Russell'][, `:=` ("pool_parent"= 'Russell', 'vcf' = paste(PedigreeItem, '_chr25_pile.vcf.gz',sep=""))]
...
```{r}
#male pool parents are all the same for each pool e.g A1, B2 etc. unless adding multiple copies of each.
##Russell_tp_bams_single <- Rualn_bams[V1 == 'ER10522_CK02_04' | V1 == 'ER10522_CK06_01' | V1 == 'ER10522_CK09_06'| V1 == 'ER10522_CK13_02'| #V1 =='ER10522_CK20_01'| V1 =='ER10522_CK51_06'| V1 =='ER10522_Russell'][, `:=` ("pool_parent"= 'Russell', 'vcf' = paste(PedigreeItem, #'_chr25_pile.vcf.gz',sep=""))]# only found "CK06_01","CK20_01","CK51_06". missing 'CK02_04', 'CK13_02','CK09_06'.
#Russell_tp_bams_x <- Russell_tp_bams_single[c(1,2,3,4,4,4,4,4,4,4)]
```
...
```{r delete after run bcftools mpileup for chr25}
submit_mpileup <- function(reference = "*.fa.gz", output = c(), bamfiles = c(), job.init = "asub", job.opts = "-q normal") {
  cmd          <- "bcftools mpileup -B -I -Q 20 -f {reference} --excl-flags 'UNMAP,SECONDARY,QCFAIL,DUP'  -r 'chr25' -a {mpileuptags} {bamfiles} -Ou | bcftools call -f {bcfcalltags} -p {pvalThres} -m -A -Oz -o {output} && bcftools index {output}" 
  tmpf         <- tempfile(pattern = "submit_mileup.", fileext = ".sh")
  writeLines(text = c(glue(cmd)), con = tmpf)
...
bcfcalltags    <- "GQ,GP"
pvalThres      <- 0.99
PedigreeItem   <- "Russell"
bamfiles       <- 'Russell_mrgdparents_sort.bam'
output         <- paste("bcfmpile_parents_Russell_chr25.vcf")
reference      <- dir("/workspace/hrards/projects/hrtcdf/genome/fasta", pattern = ".*\\.gz$", full.names = TRUE)
file.exists(bamfiles)
file.exists(reference)
submit_mpileup(reference = reference, output = output, bamfiles = bamfiles)
