...
dt <- fread("CK02_01_fullpool_dt.csv")
rm(dt)
```

```{r group for analysis rename for QTLseqR}
ck <- fread("CK02_01_fullpool_dt.csv")
...
ggplot(data = ck) +geom_histogram(aes(x = DP.HIGH + DP.LOW)) +ggtitle("CK02_01") #+xlim(0,1000)
```

```{r ref frequency, include=TRUE, fig.show="hold", fig.width= 3, fig.height=3}
ggplot(data = ck) +geom_histogram(aes(x = REF_FRQ))+ggtitle("CK02_01")
```


```{r QTLseqAnalysis}
#bulk size high bulk first, high = parent.
ck_qtl <- runQTLseqAnalysis(ck,windowSize = 2e6, popStruc = "F1", bulkSize = c(5,38), replications = 10000, intervals = c(95, 99))
```
...
ck_Hampel_filt <- plotGprimeDist(SNPset = ck_qtl, outlierFilter = "Hampel")+ggtitle("CK02_01")
ck_Hampel_filt
```

```{r plotGprimeDist deltaSNP outlier, fig.show="hold", fig.width= 5, fig.height=3}
ck_deltaSNP_filt <- plotGprimeDist(SNPset = ck_qtl, outlierFilter = "deltaSNP", filterThreshold = 0.4) +ggtitle("CK02_01")
ck_deltaSNP_filt
```

```{r plot QTLstats nSNPs, include=TRUE, fig.width= 20}
ck_plotQTLS <- plotQTLStats(SNPset = ck_qtl, var = "nSNPs")+ggtitle("CK02_01")
ck_plotQTLS
```


```{r plot QTLstats deltaSNP, include=TRUE, fig.width= 20}
ck_plotQTLSdelta <- plotQTLStats(SNPset = ck_qtl, var = "deltaSNP", plotIntervals = TRUE)+ggtitle("CK02_01")
ck_plotQTLSdelta
```
#, include=TRUE, fig.width= 3, fig.height=3
```{r plotQTLstats Gprime, include=TRUE, fig.width= 20}
ck_plotgp <- plotQTLStats(SNPset = ck_qtl, var = "Gprime", plotThreshold = TRUE, q = 0.01)+ggtitle("CK02_01")
ck_plotgp
ggsave("Gplot_CK02_01.png", plot = ck_plotgp, width = 35, height = 12, units = c("cm"), dpi = 320 )
```
...
```{r}
results <- getQTLTable(SNPset = ck_qtl, method = "Gprime", alpha = 0.01, export=FALSE)
