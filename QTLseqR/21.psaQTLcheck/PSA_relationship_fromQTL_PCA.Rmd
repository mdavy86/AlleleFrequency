...
allsites <- allsites[,"ChrPos" := paste(CHROMn,POS,sep=".")] #an issue with fread requires these to be re-writen.
...
for (i in qtl_names){
  ...
  subset_title   <- as.character(get(i)[1, c(5,9,13,17,21,25,29,33,37,41, 45,49,53,57,61,65,69,73,77,81,85,89, 93,97,101,105,109,113,117,121, 125)]) # get names of each QTL, attach sample name for bases column
...
samplepoolname  <- paste0("Sample pool (",pool,")")
PCA_of_poolQTL  <- PCA_of_poolQTL[is.na(PSA_1tol_5sus), PSA_1tol_5sus := glue(samplepoolname)] # paste0("Sample pool (",pool,")")
PCA_of_poolQTL  <- PCA_of_poolQTL[, PSA_1tol_5sus := str_replace_all(PCA_of_poolQTL$PSA_1tol_5sus, c( "^1"="Tolerant plant", "^2"="Tolerant plant", "^3"="Susceptible plant", "^4"="Susceptible plant", "^5"="Susceptible plant"))] 
...
PCA_of_poolQTLp <- function(A) {
    ggplot(PCA_of_poolQTL, aes(x=V3, y=V2, colour=QTL_name_levels, fill=QTL_name_levels,shape=as.factor(PSA_1tol_5sus)))+ #,shape=PSA_1tol_5sus
    ...
    guides(shape = guide_legend(str_wrap(paste0("PSA phenotype of parental individuals with the sample pool QTL were derived from"), 23), order=1), colour = guide_legend("QTL name", order=2), fill="none")+
      ggtitle(as.expression(bquote(atop("The contribution of alleles from pool" ~ .(pool) ~ "to PSA phenotype", "in parental" ~ italic("A. chinensis") ~ "var." ~ italic("chinensis") ~ "individuals"))))
}
#, italic("A. chinensis"), " var. ", italic("chinensis"), "individuals")
#as.expression(bquote(R^2 == .(rsq)
#as.expression(bquote(R^2~"="~.(rsq))))
p <- PCA_of_poolQTLp()
