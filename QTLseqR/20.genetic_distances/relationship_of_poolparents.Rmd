...
PCA_of_poolQTL  <- PCA_of_poolQTL[, Parent_sample :=factor(PCA_of_poolQTL$Parent_sample, levels=c("P1", "P2", "P3", "P4", "P8", "P9", "P10", "P11", "P12"))]
...
PCA_of_poolQTLp <- function(A) {
    ggplot(PCA_of_poolQTL, aes(x=PC1, y=PC2, colour=Parent_sample, fill=Parent_sample))+
    geom_point(size= 4)+ 
    scale_x_continuous(name=paste0("PC1 (",PC1_contribution," %)")) + #, breaks=seq(-0.2,0.2,0.04)
    scale_y_continuous(name=paste0("PC2 (",PC2_contribution," %)")) +
    ...
    guides(colour = guide_legend("Parent"), fill="none")
    }
p <- PCA_of_poolQTLp()
