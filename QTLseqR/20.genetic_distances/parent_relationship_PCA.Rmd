...
for (i in qtl_names){
  ...
  assign(i, setnames(get(i), paste(subset_Bases), subset_title_qtlname)) #setnames qtl_names to bases colunm 
  assign(i, t(get(i)[, subset_title_qtlname, with = FALSE]))#transpose and subset

  assign(i, as.data.table(cbind(rownames(get(i)),get(i))))#get the rownames and paste as a column
  
  assign(i, cbind(get(i), apply(get(i)[,-1], 1, paste, collapse="")))
...
sites <- read.table(file=paste0("/powerplant/workspace/hrtcdf/github/FSTs/QTLseqR/20.genetic_distances/",site_name), sep="\t",skipNul = FALSE)#read.table reads the T as true.
