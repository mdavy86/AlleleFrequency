...
mrgd      <- merge.data.table(mrgd,stackedsitesP19, by = c("ChrPos","R0A1"), all=F)
mrgd      <- merge.data.table(mrgd,stackedsitesP12, by = c("ChrPos","R0A1"), all=F)
}
if (pool=="P3"){
...
mrgd_names      <- names(mrgd[,c(5)])
mrgd_names      <- "qtl_name_P12pp"

for (i in qtl_names){
  assign(i, mrgd[mrgd_names==i]) #subset for each qtl.
...
p <- PCA_of_poolQTLp()
#ggsave(paste0("sPC_sample_QTLs",pool), plot = p)
p
