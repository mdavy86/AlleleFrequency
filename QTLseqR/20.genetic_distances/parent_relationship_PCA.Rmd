...
### PCA core
	...
	contribution_of_PCs    <- round((res_svd$d/sum(res_svd$d)*100)[2], digits=2)##added after working to get 
	write.table(contribution_of_PCs, file="contribution_of_PCs.txt", sep="\t", append=T)##added after working to get 
...
##plot each QTL seperately


#```
fwrite(sPC_sample, file="sPC_sample_QTLs.txt", sep="\t", append=F,col.names=F)## can be removed after testing
sPC_sample  <- fread(file="sPC_sample_QTLs.txt")## can be removed after testing
## plot each QTL seperately
#```{r plot each QTL seperately}
if (length(names(sPC_sample))==7){sPC_sample  <- sPC_sample[, `:=` (V5=NULL)]}# P1and P4 have 3 individuals contributing to the sample pool. adjust the number of PCA columns in these individuals to plot like the rest.
sPC_sample  <- sPC_sample[, c("s1", "s2","s3","s4","s5") := tstrsplit(V1, "_", fixed=TRUE)]#[,c(-1)]
sPC_sample  <- setnames(sPC_sample, names(sPC_sample), c("n1", "PC1" ,"PC2", "PC3", "PC4", "QTL_name", "s1", "s2", "s3", "s4", "Parent_sample"))#}
sPC_sample  <- sPC_sample[, Parent_sample := str_replace_all(Parent_sample, "sp", "b")]#change notation
#sPC_sample  <- factor(sPC_sample[,Parent_sample], levels = unique(sPC_sample$Parent_sample))

#QTL_name_levels  <- sPC_sample[, "chr_n" := str_replace_all(s2, "chr", "")][, chr_n := as.numeric(chr_n)][order(chr_n)]
#QTL_name_levels  <- factor(QTL_name_levels[,QTL_name], levels = unique(QTL_name_levels$QTL_name))

pool                <- unique(sPC_sample$s1)
QTLposition         <- as.numeric(round(as.numeric(unique(sPC_sample$s3))/1000000, digits = 1))
QTLchromosome       <- unique(sPC_sample$s2)
QTLchromosome       <- str_replace(QTLchromosome, "chr", "chromosome ")
...
sPC_samplep <- function(A) {
    ggplot(sPC_sample, aes(x=PC1, y=PC2, shape=factor(sPC_sample[,Parent_sample], levels = unique(sPC_sample$Parent_sample)), colour=factor(sPC_sample[,Parent_sample], levels = unique(sPC_sample$Parent_sample))))+ #colour=QTL_name_levels, fill=QTL_name_levels
    geom_hline(yintercept=0, color = "white", size = 0.5) +
    geom_vline(xintercept=0, color = "white", size = 0.5) +
    geom_point(size= 3) + #, alpha = 7/10
    geom_text(label=sPC_sample$Parent_sample, nudge_x = 0.005, hjust = 0, check_overlap = T)+
    scale_shape_manual(values=c(8,19,15,17,18)) +
    scale_colour_manual(values = c("black", "darkgreen", "red","blue","purple","pink")) +
    scale_x_continuous(name=paste0("PC1 (",PC1_contribution," %)"), expand = c(.125, 0)) + #breaks=seq(-0.2,0.2,0.04),
    scale_y_continuous(name=paste0("PC2 (",PC2_contribution," %)"), expand = c(.125, 0)) + #breaks=seq(-0.2,0.2,0.04),
    #theme_classic()+
    theme(plot.title   = element_text(hjust = 0, vjust = 0.5, size=11, family="Arial"),
          axis.title.y = element_text(hjust = 0.5, vjust = 0.5, size=11, family="Arial"),
          axis.title.x = element_text(hjust = 0.5, vjust = 0,   size=11, family="Arial"),
          axis.text.x  = element_text(angle = 0,   vjust = 0.5, size=11, family="Arial"),
          axis.text.y  = element_text(vjust = 0,   hjust = 0.5, size=11, family="Arial"),
   ...
   ggtitle(str_wrap(paste0("QTL from ", QTLposition, " Mb along ", QTLchromosome), 45))
  # ggtitle(str_wrap(paste0("The inheritance of significant SNPs forming QTL at " , QTLposition, " Mb", " along ", QTLchromosome," from bulk ", pool, "b"), 45))+
    #guides(shape = guide_legend(str_wrap(paste0("Sample bulk ",pool, "b (star)", " and parents contributing to bulk ",pool, "b" ), 23), order=1), colour = guide_legend("QTL name", order=2), fill="none")
    }
p <- sPC_samplep()
p
ggsave(paste0("PCA_poolparents_vs_pool_",ii,".png"), plot = p, height = 8, width=10, units = c("cm"), dpi = 320)
}
}
```
...
## plot all qtl for each pool used within the loop from above. hash the r chunk start to run.
...
PCA_of_poolQTL  <- PCA_of_poolQTL[, Parent_sample := str_replace_all(Parent_sample, "sp", "b")]#change notation

#change factors for shape plot order
if(pool=="P1"){PCA_of_poolQTL  <- PCA_of_poolQTL[, Parent_sample :=factor(PCA_of_poolQTL$Parent_sample, levels=c("P1b", "P1", "P10", "P12", "P21"))]}
if(pool=="P2"){PCA_of_poolQTL  <- PCA_of_poolQTL[, Parent_sample :=factor(PCA_of_poolQTL$Parent_sample, levels=c("P2b", "P2", "P9", "P12"))]}
if(pool=="P3"){PCA_of_poolQTL  <- PCA_of_poolQTL[, Parent_sample :=factor(PCA_of_poolQTL$Parent_sample, levels=c("P3b", "P3", "P11", "P13"))]}
if(pool=="P4"){PCA_of_poolQTL  <- PCA_of_poolQTL[, Parent_sample :=factor(PCA_of_poolQTL$Parent_sample, levels=c("P4b", "P4", "P9", "P11", "P15"))]}
if(pool=="P9"){PCA_of_poolQTL  <- PCA_of_poolQTL[, Parent_sample :=factor(PCA_of_poolQTL$Parent_sample, levels=c("P9b", "P9", "P2", "P4"))]}
if(pool=="P10"){PCA_of_poolQTL <- PCA_of_poolQTL[, Parent_sample :=factor(PCA_of_poolQTL$Parent_sample, levels=c("P10b", "P10", "P1", "P8"))]}
if(pool=="P11"){PCA_of_poolQTL <- PCA_of_poolQTL[, Parent_sample :=factor(PCA_of_poolQTL$Parent_sample, levels=c("P11b", "P11", "P3", "P4"))]}
if(pool=="P12"){PCA_of_poolQTL <- PCA_of_poolQTL[, Parent_sample :=factor(PCA_of_poolQTL$Parent_sample, levels=c("P12b", "P12", "P1", "P2"))]}
...
PCA_of_poolQTLp <- function(A) {
    ...
    theme(plot.title   = element_text(hjust = 0.5, vjust = 0.5, size=12, family="Arial"),
          ...
          legend.title = element_text(size=9, family="Arial"),
          legend.text  = element_text(size=8, family="Arial"))+
    ggtitle(str_wrap(paste0("Genomic distance among 10 Kb regions under QTL peaks from ",pool,"b bulk"), 50))+
    guides(shape = guide_legend(str_wrap(paste0("Sample bulk ",pool, "b (star)", " and each parent contributing to bulk"), 23), order=1), colour = guide_legend("QTL name", order=2), fill="none")
    }
#p <- PCA_of_poolQTLp()
