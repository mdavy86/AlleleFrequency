...
#for each pool parent get data from vcf sample files to a single data table.
...
## plot PCA one bed at a time for each pool. then compile
...
#qtl from beds and individuals contributing to the P1pool #should all these be %chin%?
...
#lapply(pred, class)
#attach names of bedfile region(qtl_names) to pred

# vcfGR  <- GRanges(seqnames = pred$CHROM, ranges=IRanges(start = pred$POS, end = pred$POS))
# bedGR  <- GRanges(seqnames = bed0$V1, ranges=IRanges(start = bed0$V2, end = bed0$V3, names = bed0$V4))
# coords <- findOverlaps(vcfGR, bedGR)
# vcfGR[queryHits(coords)]
# pred_vcf_hits <- pred[queryHits(coords),]
# bed0[subjectHits(coords),]
# pred_vcf_hits$qtl_name <- bed0[subjectHits(coords), V4]
# pred_vcf_hits <- as.data.table(pred_vcf_hits)
# pred_vcf_hits <- pred_vcf_hits[, "CHROMn" := as.numeric(str_replace_all(CHROM, "chr", ""))]
# pred_vcf_hits <- pred_vcf_hits[, "SAMPLEn" := as.factor(as.numeric(str_replace_all(SAMPLE, "P", "")))]
...
```{r transform, options(digits=9)}
fileP <- ("P12G10000_vcf_hits.csv")
sites <- fread(fileP)

sites  <- sites[AD==".", `:=` (REF="-",ALT0="-",ALT1="-")][AD1=="n", `:=` (ALT0="-")][REF=="", `:=` (REF="-")][ALT1==".", `:=` (ALT1="-")][str_length(REF)>1, `:=` (REF="-")][str_length(ALT0)>1, `:=` (ALT0="-")][str_length(ALT1)>1, `:=` (ALT1="-")]

#collapse parents ref and alt into single strand giving REF,ALT0 strands at pos 1,2. there are only 20 
#remove_uneven <- as.data.table(table(sites$poolparent_chrpos))
#ru            <- unlist(remove_uneven[N<max(N), .(V1)])
#sites         <- sites[poolparent_chrpos!=ru,]

#get nucleotide string based on seperate samples then merge stackedsitesPx with key ChrPos
#mrgd <- data.table("SAMP_ppqtlname"=character(), "Bases"=character(), "ChrPos"=numeric())
px   <- unique(sites$SAMPLE)

  
for (i in seq(along=px)){
assign(paste0("REFsites",px[i]), sites[SAMPLE==px[i],  .(poolparent, SAMPLE,  qtl_name, REF,ChrPos)][,"R0A1" :="0"])
assign(paste0("REFsites",px[i]), setnames(get(paste0("REFsites",px[i]), envir = .GlobalEnv), c("REF"), c("Bases")))
assign(paste0("ALTsites",px[i]), sites[SAMPLE==px[i],  .(poolparent, SAMPLE,  qtl_name, ALT0,ChrPos)][,"R0A1" :="1"]) 
assign(paste0("ALTsites",px[i]), setnames(get(paste0("ALTsites",px[i]), envir = .GlobalEnv), c("ALT0"), c("Bases")))
assign(paste0("stackedsites",px[i]), rbind(get(paste0("REFsites",px[i]), envir = .GlobalEnv), get(paste0("ALTsites",px[i]), envir = .GlobalEnv)))

assign(paste0("stackedsites",px[i]), get(paste0("stackedsites",px[i]), envir = .GlobalEnv)[, "SAMP_ppqtlname_R0A1" := paste(SAMPLE,qtl_name,R0A1, sep="_")][,.(SAMP_ppqtlname_R0A1, Bases, ChrPos, R0A1, qtl_name)][Bases==".",Bases:="-"])

#setkey for bind
assign(paste0("stackedsites",px[i]), setkey(get(paste0("stackedsites",px[i]), envir = .GlobalEnv), ChrPos))
#mrgd <- data.table(
tmp <- get(paste0("stackedsites",px[i]), envir = .GlobalEnv)
setkey(tmp,ChrPos)
}

##Adjust this portion manually for each stackedsitesPx parent contributing to mrgd
mrgd      <- merge.data.table(stackedsitesP1,stackedsitesP2, by = c("ChrPos","R0A1"), all=T)
mrgd      <- merge.data.table(mrgd,stackedsitesP12, by = c("ChrPos","R0A1"), all=T)





#attach the SAMP_ppqtlname_R0A1 name to sequence title instead of e.g bases.x
subset_title   <- str_subset(names(mrgd), "SAMP*")
subset_Bases   <- str_subset(names(mrgd), "Base*")
for (ix in seq(along=subset_title)){
st   <- subset_title[ix]
stt   <- mrgd[[st]]
print(stt)

#mrgd <- setnames(mrgd, c(subset_Bases[io]), c(subset_title[io]))
}

qtl_names      <- unique(mrgd$qtl_name)
for (i in qtl_names){
assign(paste(i), as.data.table(mrgd[qtl_name.x==i]))

#fwrite(mrgd_w,file=paste0("wide",i,fileP))
}
#transpose to wide for each qtl_name over all merged parents columns
```

```{r}
sites <- stackedsitesP1
...
```{r}
	...
	# checking distribution of the distances
   		distance <- dist(diffs, method = "euclidean", upper = TRUE)
   		distance <-as.matrix(distance)
   		distance <- distance/n_seq^0.5
	 hist(distance)
...
#there is no openGL in powerplant
```{r 3D presentation}
  #library("rgl")
 ...
 plot3d( sPC_sample [,1], sPC_sample[,2], sPC_sample[,3], col="black", pch=NA, main="Human", xlab="sPC1", ylab="sPC2", zlab="sPC3", type="p")
   ...
   #powerplant doesn't have directGL workaround?
```
