...
for (i in qtl_names){
  assign(i, mrgd[qtl_name_P12pp==i]) #subset for each qtl.
  subset_title    <- as.character(get(i)[1, c(5,10,15,20)]) # get names of each qtl and attach sample name for bases column
  subset_Bases    <- str_subset(names(get(i)), "Bases*")    # transpose, get old column names
  ...
  tmp <- apply(get(i)[, ..subset_title_qtlname], 2, paste, collapse="") # paste SNPs together into tmp
  assign(i, data.table(names(tmp), unname(tmp)))                        # assign to i
