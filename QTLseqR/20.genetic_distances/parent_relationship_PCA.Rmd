...
PCA_of_poolQTLp <- function(A) {
    ...
    ggtitle(str_wrap(paste0("Genomic distance among 10 Kb regions under QTL peaks in pool ",pool ), 50))+
    guides(shape = guide_legend(str_wrap(paste0("Sample pool ",pool, "sp (star)", " and parents contributing to pool"), 23), order=1), colour = guide_legend("QTL name", order=2), fill="none")
    }
p <- PCA_of_poolQTLp()
