...
if (pool=="P1"){
mrgd      <- merge.data.table(stackedsitesP1pp,stackedsitesP1, by = c("ChrPos","R0A1"), all=F)
...
if (pool=="P2"){
mrgd      <- merge.data.table(stackedsitesP2pp,stackedsitesP2, by = c("ChrPos","R0A1"), all=F)
mrgd      <- merge.data.table(mrgd,stackedsitesP9, by = c("ChrPos","R0A1"), all=F)
mrgd      <- merge.data.table(mrgd,stackedsitesP12, by = c("ChrPos","R0A1"), all=F)
}
if (pool=="P3"){
mrgd      <- merge.data.table(stackedsitesP3pp,stackedsitesP3, by = c("ChrPos","R0A1"), all=F)
...
if (pool=="P4"){
mrgd      <- merge.data.table(stackedsitesP4pp,stackedsitesP4, by = c("ChrPos","R0A1"), all=F)
mrgd      <- merge.data.table(mrgd,stackedsitesP9, by = c("ChrPos","R0A1"), all=F)
mrgd      <- merge.data.table(mrgd,stackedsitesP11, by = c("ChrPos","R0A1"), all=F)
}
if (pool=="P9"){
mrgd      <- merge.data.table(stackedsitesP9pp,stackedsitesP9, by = c("ChrPos","R0A1"), all=F)
...
if (pool=="P10"){
mrgd      <- merge.data.table(stackedsitesP10pp,stackedsitesP10, by = c("ChrPos","R0A1"), all=F)
...
if (pool=="P11"){
mrgd      <- merge.data.table(stackedsitesP11pp,stackedsitesP11, by = c("ChrPos","R0A1"), all=F)
...
if(pool=="P1"){PCA_of_poolQTL  <- PCA_of_poolQTL[, Parent_sample :=factor(PCA_of_poolQTL$Parent_sample, levels=c("P1pp", "P1", "P10", "P12"))]}
if(pool=="P2"){PCA_of_poolQTL  <- PCA_of_poolQTL[, Parent_sample :=factor(PCA_of_poolQTL$Parent_sample, levels=c("P2pp", "P2", "P9", "P12"))]}
if(pool=="P3"){PCA_of_poolQTL  <- PCA_of_poolQTL[, Parent_sample :=factor(PCA_of_poolQTL$Parent_sample, levels=c("P3pp", "P3", "P11", "P13"))]}
if(pool=="P4"){PCA_of_poolQTL  <- PCA_of_poolQTL[, Parent_sample :=factor(PCA_of_poolQTL$Parent_sample, levels=c("P4pp", "P4", "P9", "P11"))]}
if(pool=="P9"){PCA_of_poolQTL  <- PCA_of_poolQTL[, Parent_sample :=factor(PCA_of_poolQTL$Parent_sample, levels=c("P9pp", "P9", "P2", "P4"))]}
if(pool=="P10"){PCA_of_poolQTL <- PCA_of_poolQTL[, Parent_sample :=factor(PCA_of_poolQTL$Parent_sample, levels=c("P10pp", "P10", "P1", "P8"))]}
if(pool=="P11"){PCA_of_poolQTL <- PCA_of_poolQTL[, Parent_sample :=factor(PCA_of_poolQTL$Parent_sample, levels=c("P11pp", "P11", "P3", "P4"))]}
if(pool=="P12"){PCA_of_poolQTL <- PCA_of_poolQTL[, Parent_sample :=factor(PCA_of_poolQTL$Parent_sample, levels=c("P12pp", "P12", "P1", "P2"))]}
#example to remove df$var2 <- factor(df$var2, levels=c("P1pp", "P1", "P10", "P12"))

pool            <- names(fread("tmp_pool"))

PCA_of_poolQTLp <- function(A) {
    ggplot(PCA_of_poolQTL, aes(x=PC2, y=PC1, colour=QTL_name, shape=Parent_sample))+#fill=s5
    geom_point(size= 4)+  # shape = 21, alpha = 3/10, 
    scale_shape_manual(values=c(16,1,3,4)) +    
    #ggtitle(paste0(pool,"sPC_sample_QTLs")) +
    theme_classic()+
    theme(plot.title   = element_text(hjust = 0.5, vjust = 0.5, size=14),
          axis.title.y = element_text(hjust = 0.5, vjust = 0.5, size=12),
          axis.title.x = element_text(hjust = 0.5, vjust = 0,   size=12),
          axis.text.x  = element_text(angle = 0,   vjust = 0.5, size=12),
          axis.text.y  = element_text(vjust = 0,   hjust = 0.5, size=12))+
    guides(shape = guide_legend(str_wrap(paste0("Sample pool ",pool, "pp (colour filled)", " and parents contributing to pool (no fill)"), 23), order=1), color = guide_legend("QTL name", order=2))
    }
p <- PCA_of_poolQTLp()
ggsave(paste0("PCA_poolparents_vs_pool_",pool,".png"), plot = p)
p
}
```

#### output to png images generated by Tomokazu Konishi
```{r}
# sample
