...
compare_mpool  <- compare_mpool[V4=="A"]#too big to run all at once. cycle through AB,CD,EF,GHIJ manually
```
...
```{r ggplot, eval=TRUE, warnings=TRUE, include=TRUE, fig.show="hold", fig.width= 15}
for(i in seq(along=obj_names)) {
...
dt_name <- assign(obj_names[i], dt_name[negLog10Pval>quant, "quant0.05":="Top 1%(q<0.05)"][negLog10Pval<quant, quant0.05:="Remaining SNPs"])
q <- max(dt_name[qvalue<0.05, negLog10Pval],na.rm=T, warning=F)
...
geom_point(aes(color=factor(dt_name[,quant0.05])),size=0.5, na.rm=T)+
scale_color_manual(name="QTL",breaks = c("Top 1%(q<0.05)","Remaining SNPs"),values=c("chartreuse3","black"))+
...
#ggsave(paste0(obj_names[i],"_plotgp.png"), plot = last_plot(), width = 20, height = 12, units = c("cm"), dpi = 72)
}
```