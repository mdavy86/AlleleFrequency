...
#BSAsnps <- BSAsnps[, parent := str_replace_all(parent, c("sp_bsa" = "sp_bsa"))]
...
#give second Gp_max at max GP for P3 chr11 which has 2 peaks on the same chr to split


p <- ggplot(data=BSAsnps_peakpoint, aes(x=Chromosome, y=POSmb))+
geom_point(aes(colour=BSAsnps_peakpoint[,parent], fill=factor(BSAsnps_peakpoint[,parent]), shape=factor(BSAsnps_peakpoint[,overlap])), na.rm=T, alpha=0.8, size = 5, position =position_jitter(seed = 43))+# ,size=BSAsnps_peakpoint[,overlap]
