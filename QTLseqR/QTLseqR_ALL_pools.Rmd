...
```{r ggplot qtls, eval=TRUE, warnings=FALSE, include=TRUE, fig.show="hold", fig.width= 15}
for(i in seq(along=obj_names)) {
...
#dt_name <- dt_name[qvalue<0.05,]
#fwrite(dt_name,file="allelefreq_sig_snps.csv", append=T)
...
ggsave(paste0(obj_names[i],"_plotgp.png"), plot = last_plot(), width = 20, height = 12, units = c("cm"), dpi = 320)
}
```

```{r write threshold snps}
for(i in seq(along=obj_names)) {
dt_name <- as.data.table(get(obj_names[i], envir = .GlobalEnv))
dt_name <- dt_name[qvalue<0.05,]
...
threshold_snps <- fread("allelefreq_sig_snps.csv")
```
