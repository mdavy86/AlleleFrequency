...
obj_names  <- mpool[,.(V1)]
obj_rename <- obj_names[V1=='CK02_01_samplepoolSNPs_dt.csv', 'rename' :="P1sp_bsa"][V1=='CK10_02_samplepoolSNPs_dt.csv', rename :="P9sp_bsa"][V1=='CK19_03_samplepoolSNPs_dt.csv', rename :="P2sp_bsa"][V1=='CK20_01_samplepoolSNPs_dt.csv', rename :="P10sp_bsa"][V1=='CK22_03_samplepoolSNPs_dt.csv', rename :="P11sp_bsa"][V1=='CK23_08_samplepoolSNPs_dt.csv', rename :="P3sp_bsa"][V1=='CK51_01_samplepoolSNPs_dt.csv', rename :="P12sp_bsa"][V1=='CK51_09_samplepoolSNPs_dt.csv', rename :="P4sp_bsa"]
...
for(i in seq(along=obj_names)) {
  ...
  assign(obj_names[i], setnames(dt_name, glue("{samplesold}"), glue("{samplesnew}"),skip_absent=TRUE))
}
```
...
##store each BSA as a file by chromosome for later access.
```{r store by chromosome to analysed_files}
chrs <- c("chr1", "chr2", "chr3",  "chr4",  "chr5",  "chr6",  "chr7",  "chr8",  "chr9", "chr10", "chr11", "chr12", "chr13", "chr14", "chr15", "chr16", "chr17", "chr18", "chr19",  "chr20", "chr21", "chr22", "chr23", "chr24", "chr25", "chr26", "chr27", "chr28", "chr29")
for(i in seq(along=obj_names)) {
  for (ic in seq(along=chrs)){ 
dt_name   <- as.data.table(get(obj_names[i], envir = .GlobalEnv))
dt_name_single_chr <- dt_name[CHROM==chrs[ic],] #get the right name for CHR
fwrite(dt_name_single_chr, file=paste0("/powerplant/workspace/hrtcdf/github/FSTs/QTLseqR/analysed_files/",obj_names[i],"_", chrs[ic],".csv"))
  }
}
```
...
shape2 <- c("P1_bsa" = 0, "P2_bsa" = 1,"P3_bsa" =2,"P4_bsa"=3, "P9_bsa"= 4, "P10_bsa" = 5,  "P11_bsa" = 6 ,"P12_bsa"=18)
...
geom_point(aes(colour=factor(parent), shape=factor(parent)), na.rm=T, size = 4, stroke = 1)+# ,size=BSAsnps_peakpoint[,overlap], , fill=factor(BSAsnps_peakpoint[,parent]), alpha=0.8,   , position =position_jitter(seed = 45, width=0.3)
...
theme_bw()+
theme(plot.title =element_text(color = "grey20", size = 11, angle = 0, hjust = .5, vjust = 0, face = "plain"),
  legend.text    =element_text(color = "grey20", size = 11, angle = 0, hjust = 0, vjust = .5, face = "plain"),
  legend.title   =element_text(color = "grey20", size = 11, angle = 0, hjust = 0, vjust = 0, face = "plain"),
  axis.text.x    =element_text(color = "grey20", size = 11, angle = 0, hjust = .5, vjust = 0, face = "plain"),
  axis.text.y    =element_text(color = "grey20", size = 11, angle = 0, hjust = 0, vjust = 0, face = "plain"),  
...
mp <-  mp[, qtl := str_replace_all(qtl, c("QTL"="QTL detected on chromosome 25"),c("no QTL"="no QTL detected on chromosome 25"))]
mp1 <- mp[1:36,]
...
geom_smooth(method=lm, se=FALSE)+#, level=0.99
  ...
  stat_poly_line() +                                #put line equation on graph
  stat_poly_eq(aes(label = after_stat(eq.label))) + #put line equation on graph
  stat_poly_eq(label.y = 0.9) +                     #put line equation on graph
  scale_y_continuous(labels = function(x) paste0(x, "%"), breaks = seq(0, 80, by = 10))+
  labs(title =str_wrap("QTL detection using the top 0.5 % of Gprime values as a threshold",100))+
  theme(plot.title =element_text(color = "grey20", size = 12, angle = 0, hjust = .5, vjust = 0, face = "plain"),
  legend.text    =element_text(color = "grey20", size = 12, angle = 0, hjust = 0, vjust = .5, face = "plain"),
  ...
  axis.text.y    =element_text(color = "grey20", size = 12, angle = 0, hjust = 0, vjust = 0, face = "plain"),  
  axis.title.x   =element_text(color = "grey20", size = 12, angle = 0, hjust = .5, vjust = 0, face = "plain"),
  axis.title.y   =element_text(color = "grey20", size = 12, angle = 90, hjust = .5, vjust = 2, face = "plain"))+
  guides(size = "none")+
  #labs(colour=str_wrap("QTL detected on chromosome 25",200),fill=str_wrap("QTL detected on chromosome 25",200))+
  xlab("Bulk number")+
  ylab(str_wrap("The expected difference in sex locus allele frequency between bulk pairs",37))
#print(p)

ggsave("Perc_dif_between_bulks_top5.png", plot = p, width = 15, height = 13, units = c("cm"), dpi = 320)
...
geom_smooth(method=lm, se=FALSE)+#, level=0.99
  ...
  axis.title.x   =element_text(color = "grey20", size = 12, angle = 0, hjust = .5, vjust = 0, face = "plain"),
  axis.title.y   =element_text(color = "grey20", size = 12, angle = 90, hjust = .5, vjust = 2, face = "plain"))+
  guides(size = "none", color=guide_legend(override.aes=list(fill=NA)))+
  #labs(colour=str_wrap("QTL detected on chromosome 25",200),fill=str_wrap("QTL detected on chromosome 25",200) )+
  xlab("Bulk number")+
  ylab(str_wrap("The expected difference in sex locus allele frequency between bulk pairs",37))
#print(p)

ggsave("Perc_dif_between_bulks_p05.png", plot = p, width = 15, height = 13, units = c("cm"), dpi = 320)
