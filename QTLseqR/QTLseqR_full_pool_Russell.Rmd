...
for (i in chrs){
s1<- fread(paste0("bcfmpile_samples_Russell_chr25.vcf_", glue({i})), col.names = c('s1CHROM','s1POS','s1REF','s1ALT','s1AD0','s1AD1','s1DP','s1GQ','s1PL'))
s1 <- s1[, "Cpos" := str_c(glue({i}), s1POS, sep=".")]
setkey(s1, Cpos)
p1 <- fread(paste0("bcfmpile_parents_Russell_chr25.vcf_", glue({i})), col.names = c('p1CHROM','p1POS','p1REF','p1ALT','p1AD0','p1AD1','p1DP','p1GQ','p1PL'))
...
fwrite(dt, file="Russell_fullpool_dt.csv", append=FALSE)
rm(dt)
```

```{r group for analysis rename for QTLseqR}
ck <- fread("Russell_fullpool_dt.csv")
...
```{r filter}
ck <- filterSNPs(SNPset = ck_nofilt, refAlleleFreq = 0.10, minTotalDepth = 40, maxTotalDepth = 85, depthDifference = 100, minSampleDepth = 10, minGQ = 100, verbose = TRUE)

```

```{r depth plot, include=TRUE, fig.show="hold", fig.width=3, fig.height=3}
ggplot(data = ck) +geom_histogram(aes(x = DP.HIGH + DP.LOW)) +ggtitle("Russell") #+xlim(0,1000)
```

```{r ref frequency, include=TRUE, fig.show="hold", fig.width= 3, fig.height=3}
ggplot(data = ck) +geom_histogram(aes(x = REF_FRQ))+ggtitle("Russell")
```


```{r QTLseqAnalysis}
ck_qtl <- runQTLseqAnalysis(ck,windowSize = 2e6, popStruc = "F1", bulkSize = c(3,50), replications = 10000, intervals = c(95, 99))
```
...
ck_Hampel_filt <- plotGprimeDist(SNPset = ck_qtl, outlierFilter = "Hampel")+ggtitle("Russell")
ck_Hampel_filt
```

```{r plotGprimeDist deltaSNP outlier, eval=FALSE, fig.show="hold", fig.width= 5, fig.height=3}
ck_deltaSNP_filt <- plotGprimeDist(SNPset = ck_qtl, outlierFilter = "deltaSNP", filterThreshold = 0.4) +ggtitle("Russell")
ck_deltaSNP_filt
```

```{r plot QTLstats nSNPs, include=FALSE, fig.width= 20}
ck_plotQTLS <- plotQTLStats(SNPset = ck_qtl, var = "nSNPs")+ggtitle("Russell")
ck_plotQTLS
```


```{r plot QTLstats deltaSNP, include=TRUE, fig.width= 20}
ck_plotQTLSdelta <- plotQTLStats(SNPset = ck_qtl, var = "deltaSNP", plotIntervals = TRUE)+ggtitle("Russell")
ck_plotQTLSdelta
```
#, include=TRUE, fig.width= 3, fig.height=3
```{r plotQTLstats Gprime, include=TRUE, fig.width= 20}
ck_plotgp <- plotQTLStats(SNPset = ck_qtl, var = "Gprime", plotThreshold = TRUE, q = 0.01)+ggtitle("Russell")
ck_plotgp
ggsave("Gplot_Russell.png", plot = ck_plotgp, width = 35, height = 12, units = c("cm"), dpi = 320)
```
