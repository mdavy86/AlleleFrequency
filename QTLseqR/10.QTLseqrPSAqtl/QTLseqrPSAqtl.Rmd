...
vcf_files_1 <- list.files(path="/powerplant/output/genomic/plant/Actinidia/chinensis/Resequencing/Variants/Russell_V2a.chromosomes.and.unassigned/ER10522", pattern="vcf.gz$", full.names=TRUE)

vcf_files_2 <- list.files(path="/powerplant/output/genomic/plant/Actinidia/chinensis/Resequencing/Variants/Russell_V2a.chromosomes.and.unassigned/ER10400_and_SRA", pattern="vcf.gz$", full.names=TRUE)
vcf_files <- c(vcf_files_1,vcf_files_2)
vcf_names <- c("vcf_1","vcf_2")

BSAallsnps          <- fread(file.path(here('QTLseqR'), "BSA0.5snps.csv"), nrows=ifelse(DEBUG, 10000, Inf))  # or BSA0.05snps.csv
...
list_chrbeds_parent  <- list.files(path=here("QTLseqR"), pattern="BSAregions_G5000_", full.names=TRUE)
...
newnames <- c("SAMPLE","CHROM", "POS", "REF", "ALT0", "ALT1", "ALT2","AD", "AD0", "AD1", "poolparent","poolparent_chrpos")
BSAallsnps_fromvcf <- setnames(BSAallsnps_fromvcf,
                               c(paste0("V", 1:10),"poolparent", "poolparent_chrpos"),
                               newnames
                               )
