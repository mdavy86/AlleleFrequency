...
G5000 <- G5000[, "CHROMn" := as.numeric(str_replace_all(CHROM, "chr", ""))]
wrap_legendtitle <- str_wrap(paste("Parent contributing", paste0("to pool ",i)),20)
wrap_ylab <- str_wrap(paste0("Number of SNPs identical to QTL peak apex +/- 10000 bp"),38)

p <- ggplot(data=G5000, aes(x=reorder(qtl_name,CHROMn),qtl_name, y=sumSNPs_bysample_byqtlname))+
geom_jitter(aes(colour=G5000[,SAMPLE]),alpha=5/10,shape=20, na.rm=T)+
