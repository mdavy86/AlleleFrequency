...
list_chrbeds_parent  <- grep(pattern="BSAregions_G10000_", list.files(path=here("QTLseqR"), full.names=T), invert=F, value=T)
names_chrbeds_parent <- str_replace_all(list_chrbeds_parent, c("/powerplant/workspace/hrtcdf/github/FSTs/QTLseqR/BSAregions_"="", "_bsa.bed"="", "G10000_"=""))
...
fwrite(BSAallsnps_fromvcf, file = "g10000_BSAgSNPs.csv")
...
G5000 <-fread(paste0("/powerplant/workspace/hrtcdf/github/FSTs/QTLseqR/10.QTLseqrPSAqtl/prediction_G50000_",{i},".csv"))
bed0  <- fread(paste0("/powerplant/workspace/hrtcdf/github/FSTs/QTLseqR/BSAregions_G50000_",i,"_bsa.bed"))
...
wrap_ylab <- str_wrap(paste0("Fraction of alternate SNPs identical to a 100000 bp region under the QTL peak apex"),32)
...
theme(plot.title =element_text(color = "grey20", size = 12, angle = 0, hjust = .5, vjust = 0, face = "plain"),
  ...
  ylab(wrap_ylab)
print(p)
ggsave(paste0(i,"_colourparentsQTLpeakoverlap100000.png"), plot = last_plot(), width = 20, height = 12, units = c("cm"), dpi = 320)
}
```
