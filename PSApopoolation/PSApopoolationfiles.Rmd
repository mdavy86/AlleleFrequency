...
**parent files pulled from database, merged with picard and piled with bcftools since samtools/0.1.18 keeps having issues with read groups**
...
```{r  merge vcfs into parent pools- dont use samtools merge}
...
```{r merge bams into parent pools}
for (i in unique(parent_bams$pool_parent)){ 
x       <- parent_bams[pool_parent=={i}, pbams]
...
file.exists(x)
vcfcmd         <- glue("java -jar /software/bioinformatics/picard-tools-2.9.4/picard.jar MergeSamFiles {infile} O={outfile}")
OutEr          <- "picardmrg1"; logD<-here("logD"); bsO<-file.path(logD,paste0(OutEr,".out")); bsE<-file.path(logD,paste0(OutEr,".err"))
bsub_cmd       <- glue("bsub -n 8 -o {bsO} -e {bsE} {vcfcmd}")
system(bsub_cmd)
}
```

```{r picard AddOrReplaceReadGroups replace read groups- probably not needed as RG retained by pickard merge.}
...
vcfcmd  <- glue("java -jar /software/bioinformatics/picard-tools-2.9.4/picard.jar AddOrReplaceReadGroups I= {i} O= {f}_rg.bam RGID=null RGLB=null RGPL=SANGER RGPU=null RGSM={f}")
OutEr          <- "picardrg3"; logD<-here("logD"); bsO<-file.path(logD,paste0(OutEr,".out")); bsE<-file.path(logD,paste0(OutEr,".err"))
bsub_cmd       <- glue("bsub -n 8 -o {bsO} -e {bsE} {vcfcmd}")
system(bsub_cmd)
}
```
...
submit_mpileup <- function(reference = "*.fa.gz", output = c(), bamfiles = c(), job.init = "asub", job.opts = "-q normal") {
  cmd          <- "bcftools mpileup -B -I -Q 20 -f {reference} --excl-flags 'UNMAP,SECONDARY,QCFAIL,DUP'  -r 'chr25' -a {mpileuptags} {bamfiles} -Ou | bcftools call -f {bcfcalltags} -p {pvalThres} -m -A -Oz -o {output} && bcftools index {output} && bcftools +fill-tags {output} -Oz -o {filltagsoutput} -t {filltag}" 
  tmpf         <- tempfile(pattern = "submit_mileup.", fileext = ".sh")
  writeLines(text = c(glue(cmd)), con = tmpf)
  submit       <- "{job.init} {job.opts} {tmpf}"
  system(glue(submit), intern = TRUE)
}
filltag        <- "INFO/AF,INFO/AC,INFO/AN,INFO/TYPE,INFO/AC_Het"
mpileuptags    <- "DP,AD,SP,INFO/AD"
bcfcalltags    <- "GQ,GP"
pvalThres      <- 0.99
PedigreeItem   <- unique(parent_bams[, pool_parent])
bamfiles       <- unique(parent_bams[, mrgedparents])
filltagsoutput <- paste("bcfmpile_parents", PedigreeItem, "ft_chr25.vcf", sep = "_")
...
file.exists(x)
vcfcmd         <- glue("java -jar /software/bioinformatics/picard-tools-2.9.4/picard.jar MergeSamFiles {infile} O={outfile}")
OutEr          <- "picardmrg2"; logD<-here("logD"); bsO<-file.path(logD,paste0(OutEr,".out")); bsE<-file.path(logD,paste0(OutEr,".err"))
bsub_cmd       <- glue("bsub -n 8 -o {bsO} -e {bsE} {vcfcmd}")
system(bsub_cmd)
}
```
...
picmrg_lowPSApooldt_bam <- picmrg_lowPSApooldt_bam[, `:=` (sample= str_replace_all(V1, "_picmrg_lowPSA.bam", ""), "ftype"=".bam")]
...
Ox        <- "i298";Ol<-paste0(paste0(here(),"/logD/"),paste0(Ox,".out"));Oe<-paste0(paste0(here(),"/logD/"),paste0(Ox,".err"))
bsub_cmd  <- glue("bsub -n 8 -o {Ol} -e {Oe} {cmd}")
system(bsub_cmd)
}
```
```{incompatable r pools for bcftools mpileup using samtools/0.1.18. which will be run through mpileup2sync.jar- incompatable}
module("unload samtools")
module ("load samtools/0.1.18")
...
bamfiles    <- mpilepoolbams[ftype==".bam",V1]
file.exists(bamfiles)
...
```
#&& bcftools +fill-tags {output} -Oz -o {filltagsoutput} -t {filltag} && bcftools index -f {filltagsoutput}
```{r run bcftools mpileup for chr25 for QTLseqR}
submit_mpileup <- function(reference = "*.fa.gz", output = c(), bamfiles = c(), job.init = "asub", job.opts = "-q normal") {
...
bcfcalltags    <- "GQ,GP"
pvalThres      <- 0.99
PedigreeItem   <- picmrg_lowPSApooldt_bam[ftype==".bam",sample]
bamfiles       <- picmrg_lowPSApooldt_bam[ftype==".bam",V1]
filltagsoutput <- paste("bcfmpile_samples", PedigreeItem, "ft_chr25.vcf", sep = "_")
...
```{r to get filltags to go complete this.}
#vcfin         <- paste("bcfmpile_samples", PedigreeItem, "chr25.vcf", sep = "_")
#for (i in vcfin){
#vcfin          <- paste("bcfmpile_samples", PedigreeItem, "chr25.vcf", sep = "_")
filltagsoutput <- paste("bcfmpile_samples", PedigreeItem, "ft_chr25.vcf", sep = "_")
filltag        <- "AF,AC,AN,HWE,MAF"
cmd       <- glue::glue("bcftools +fill-tags 'bcfmpile_samples_D6_ft_chr25.vcf' -Ob -o 'filltagstest' -t {filltag}")
#cmd       <- glue::glue("bcftools +fill-tags {vcfin} -Oz -o {filltagsoutput} -t {filltag}")
Oi        <- "ft351";Ol<-paste0(paste0(here(),"/logD/"),paste0(Oi,".out"));Oe<-paste0(paste0(here(),"/logD/"),paste0(Oi,".err"))
bsub_cmd  <- glue("bsub -n 8 -o {Ol} -e {Oe} {cmd}")
system(bsub_cmd)
#}
```
